...
class ModelService:
    """Manage model provider configuration and the active selection."""
    ...
    @staticmethod
    async def fetch_model_names(api_url: str, api_key: str) -> list[str]:
        """Fetch model names from an Anthropic/OpenAI-compatible provider."""
        normalized_url = ModelService._validate_text(api_url, "API 地址", 2048)
        normalized_key = ModelService._validate_secret(api_key)
        endpoint = ModelService._models_endpoint(normalized_url)
        headers = {
    ...
    async def save_model_site(
        ...
        normalized_original = ModelService._validate_optional_name(original_name)
        normalized_name = ModelService._validate_name(name, "供应商名称")
        normalized_url = ModelService._validate_text(api_url, "API 地址", 2048)
        normalized_key = ModelService._validate_secret(api_key)
        normalized_models = ModelService._validate_models(models)
        normalized_protocol = ModelService._validate_api_protocol(api_protocol)
        ...
        ModelService._restore_current_selection(settings)

    @staticmethod
    async def delete_model_site(name: str) -> None:
    ...
    async def get_current_model() -> ModelSelection | None:
        ...
        selection = ModelService._find_selection(
        ...
        for site in settings.model:
            if site.models:
                ...
                ModelService._write_selection(selection)
                return selection
        return None

    @staticmethod
    async def set_current_model(site: str, model: str) -> None:
        ...
        settings = await get_settings()
        selection = ModelService._find_selection(settings, site, model)
        if selection is None:
            raise ValueError("所选模型不在当前配置中")
        ModelService._write_selection(selection)

    @staticmethod
    def _restore_current_selection(settings: Settings) -> None:
        ...
        for site in settings.model:
            if site.models:
                ModelService._write_selection({"site": site.name, "model": site.models[0].name})
                return
        ModelService._write_selection({"site": "", "model": ""})
    ...
    def _validate_optional_name(value: str | None) -> str | None:
        ...
        if value is None:
            return None
        return ModelService._validate_name(value, "原供应商名称")
    ...
    @staticmethod
    def _validate_models(value: object) -> list[ModelInfo]:
