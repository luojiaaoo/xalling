...
class FileService:
    ...
    @staticmethod
    def save_attachment(filename: str, data: str) -> dict[str, str]:
    ...
    def search_project_files(
        ...
        if not normalized_query:
            return FileService._list_top_level(root, limit)
        return FileService._search(root, normalized_query, limit)

    @staticmethod
    def _list_top_level(root: Path, limit: int) -> list[dict[str, object]]:
        entries = sorted(
            (
                entry
        ...
        return [FileService._describe(root, entry) for entry in entries[:limit]]
        ...
        for dirpath, dirnames, filenames in os.walk(root):
            ...
            for name in (*dirnames, *filenames):
                ...
                if lowered_name.startswith(query):
                    ...
                    continue
                scored.append((score, relative, FileService._describe(root, entry)))
            if scanned > MAX_SCANNED_ENTRIES:
                break
