...
def test_client_settles_usage_when_terminal_snapshot_omits_usage() -> None:
    anyio.run(_client_settles_usage_when_terminal_snapshot_omits_usage)


async def _client_settles_usage_when_terminal_snapshot_omits_usage() -> None:
    _FakeSDK.batches = [
        [
            AssistantMessage(
                content=[TextBlock(text="Thinking")],
